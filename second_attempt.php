...
<?php  $username=$_GET['username']; ?>
    <form name="cust_det" id="cust_det" method="POST" action="validate_sec_attempt.php?username=<?php echo $username ?>">
    
      <input type="hidden" name="hash" id="hash" value="<?php echo microtime(); ?>" />
     <br />  <br />
     
    <em><strong> <?php if ($_GET['attempt']==1) { echo ' Η σειρά σας έχει περάσει.Εάν θέλετε να ξαναπάρετε θέση στην ουρά πατήστε "Συνέχεια" : '; }else
	 { echo ' Δεν έχετε το δικαίωμα να ξαναπάρετε θέση στην ουρά. ' ;} ?>
	   </strong></em> <br />  <br />
       <br /> <br />
 
     <?php if ($_GET['attempt']==1) { $username=$_GET['username'];  $query_string = '?username=' . $username;
	 echo  '     <input type="submit" name="submit" class="button" value="Καταχώρηση" />' ; 
	  } ?>
     <?php if ($_GET['attempt']==2) {  echo  '  <a class="button" href="index.php"> Πίσω </a> ' ; } ?>
    </form>
     <br />
 <?php if ($_GET['attempt']==1) { echo  '  <a class="button" href="index.php"> Πίσω </a> ' ; }?>
 <!-- InstanceEndEditable -->
