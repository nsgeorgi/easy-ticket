...
//	echo "<h3 class= 'text' >Φόρμα Εγγραφής</h3>";
	...
	$query = mysql_query("SELECT *  FROM users WHERE email = '$trimmed[email]'");
...
if ($results[$email]) {
 ...
 // echo 'The email addresses you entered is not valid';
	...
	*/
	...
	$query = mysql_query("SELECT *  FROM users WHERE username = '$trimmed[user_name]'");
	$row = mysql_fetch_array($query);			
...
header('HTTP/1.1 303 See Other');
	...
	if (empty($errors)) { // Αν δεν υπάρχουν σφάλματα
		...
		$q = "INSERT INTO users (first_name, last_name, email, username, password) VALUES ('$fn', '$ln', '$e', '$u', MD5('$p'))"; // χρήση της συνάρτησης MD5 για το συνθηματικό
   ...
   $next_page = 'registration.php';    // public service einai !!
